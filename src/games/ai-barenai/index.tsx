...
function Composer({ busy, value, onChange, onSubmit, label, placeholder, submitLabel, maxLength }: { busy: boolean; value: string; onChange: (value: string) => void; onSubmit: () => void; label: string; placeholder: string; submitLabel: string; maxLength: number }) {
  const inputId = `aib-composer-${useId().replace(/:/g, '')}`;
  return <form className="aib-composer" onSubmit={(event) => { event.preventDefault(); onSubmit(); }}><div className="aib-composer__field"><label className="aib-composer__label" htmlFor={inputId}>{label}</label><input disabled={busy} id={inputId} className="aib-composer__input" value={value} onChange={(event) => onChange(event.target.value)} maxLength={maxLength} required placeholder={placeholder} /></div><PendingButton busy={busy} className="button-primary aib-composer__submit" type="submit">{submitLabel}</PendingButton></form>;
}
...
export function AiBarenaiGame({ roomState, myUserId, onBackToLobby }: Props) {
  ...
  const { pending: busy, acquire, release } = useActionLock();
  ...
  const act = async (fn: () => Promise<unknown>) => { if (!acquire()) return; setError(''); try { await fn(); } catch (caught) { setError(caught instanceof Error ? caught.message : '操作に失敗しました'); } finally { release(); } };
  ...
  if (gameState.phase === 'rule_setting') {
    ...
    bottomAction = isHost ? <div className="aib-rule-controls"><div className="aib-stepper"><span>1ラウンドのヒント担当</span><div><button type="button" aria-label="ヒント担当を減らす" disabled={busy || selectedHints <= 1} onClick={() => setHints((value) => Math.max(1, value - 1))}>−</button><strong>{selectedHints}人</strong><button type="button" aria-label="ヒント担当を増やす" disabled={busy || selectedHints >= maxHints} onClick={() => setHints((value) => Math.min(maxHints, value + 1))}>＋</button></div></div><PendingButton busy={busy} className="button-primary aib-full-button" type="button" onClick={() => void act(() => handleInitialize(selectedHints))}>ゲームを始める</PendingButton></div> : <p className="aib-waiting">ホストがルールを設定しています…</p>;
  } else if (gameState.phase === 'hinting') {
    content = <section className="aib-phase-stage"><div className="aib-stage-heading"><div><p className="section-kicker">HINTING</p><h1>ヒントを積み上げよう</h1></div><span className="aib-progress">{gameState.submittedHintPlayerIds.length}/{gameState.currentAssigneeIds.length} 提出</span></div><div className="aib-role-card">{isAnswerer ? <><strong>あなたは回答者</strong><span>お題は見えません。みんながヒントを出すのを待ちましょう。</span></> : isAssignee ? <><strong>あなたはヒント担当</strong><span>{submitted ? 'このラウンドは提出済みです。' : 'お題を確認して、ヒントを1つ出してください。'}</span>{topic ? <p className="aib-topic">お題：{topic}</p> : <PendingButton busy={busy} className="button-secondary aib-inline-button" type="button" onClick={() => void act(handleTopic)}>お題を確認する</PendingButton>}</> : <><strong>今回はヒント担当ではありません</strong><span>{assigneeNames || '担当者'}がヒントを出しています。</span></>}</div><div className="aib-hint-waiting"><span>担当者</span><strong>{assigneeNames || '未決定'}</strong></div></section>;
    if (isAssignee && !submitted) composer = <Composer busy={busy} value={text} onChange={setText} onSubmit={submitHint} label="ヒント（お題を直接言わない）" placeholder="ヒントを入力（お題を直接言わない）" submitLabel="提出" maxLength={300} />;
    ...
    if (isAnswerer && !gameState.humanAnswerSubmitted) composer = <Composer busy={busy} value={text} onChange={setText} onSubmit={submitAnswer} label="お題の答え" placeholder="お題の答え" submitLabel="回答する" maxLength={200} />;
  } else if (gameState.result) {
    content = <ResultPanel result={gameState.result} phase={gameState.phase} onComment={() => setCommentOpen(true)} />;
    if (gameState.phase === 'revealing') bottomAction = isHost ? <PendingButton busy={busy} className="button-primary aib-full-button" type="button" onClick={() => void act(handleNextRound)}>次のラウンドへ</PendingButton> : <p className="aib-waiting">ホストが次のラウンドを始めるまでお待ちください。</p>;
    if (gameState.phase === 'game_over') bottomAction = isHost ? <PendingButton busy={busy} className="button-secondary aib-full-button" type="button" onClick={() => void act(onBackToLobby)}>ロビーに戻る</PendingButton> : <p className="aib-waiting">ホストがロビーに戻るまでお待ちください。</p>;
  } else {
    content = <section className="aib-phase-stage"><p className="aib-muted">結果を読み込んでいます…</p></section>;
  }

  return <div className="ai-barenai-game" data-phase={gameState.phase}>{hud}{historyButton && <div className="aib-toolbar">{historyButton}</div>}<main className="aib-main"><div className="aib-stage">{content}</div>{composer && <div className="aib-bottom-composer">{composer}</div>}{bottomAction && <div className="aib-bottom-action">{bottomAction}</div>}{showLobbyLink && <PendingButton busy={busy} className="aib-lobby-link" type="button" onClick={() => void act(onBackToLobby)}>ロビーに戻る</PendingButton>}{error && <p className="aib-error" role="alert">{error}</p>}</main><PlayersDialog open={playersOpen} onClose={() => setPlayersOpen(false)} players={roomState.players} myUserId={myUserId} /><HistoryDialog open={historyOpen} onClose={() => setHistoryOpen(false)} hints={gameState.revealedHintHistory} answers={answerHistory} playerName={playerName} /><Dialog open={commentOpen} title="AIの感想" onClose={() => setCommentOpen(false)}><p className="aib-comment-full">{gameState.result?.aiComment}</p></Dialog></div>;
}
