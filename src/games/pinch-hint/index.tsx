...
export function PinchHintGame({ roomState, myUserId, onBackToLobby, onRefreshRoom, headerActions }: Props) {
  ...
  useEffect(() => {
    if (busyRef.current || !isPresenter || state.phase !== 'turn_setup' || typeof window === 'undefined') return;
    const draftKey = getPinchDraftKey(roomState.id, state.matchId, state.turnIndex, myUserId);
  ...
  const toggleItem = (id: string) => {
    if (busyRef.current || !isPresenter || state.phase !== 'turn_setup') return;
    ...
    <GameWrapper headerActions={headerActions} players={roomState.players} myUserId={myUserId}>
      <section className="pinch-game mx-auto w-full max-w-2xl border-2 border-[var(--line)] bg-[var(--surface)] p-4 shadow-[5px_5px_0_var(--line)] sm:p-7">
        ...
        <div className="mt-6 border-2 border-[var(--line)] bg-[var(--yellow)] p-4 sm:p-6">
          ...
          <div className="mt-5 grid grid-cols-3 gap-2">{([1, 2, 3] as const).map((value) => <button key={value} type="button" disabled={!isHost || busy} onClick={() => setRounds(value)} aria-pressed={rounds === value} className={`min-h-12 border-2 border-[var(--line)] px-2 py-3 text-lg font-black ${rounds === value ? 'bg-[var(--orange)] text-white' : 'bg-white'} disabled:cursor-not-allowed disabled:opacity-60`}>{value}ラウンド</button>)}</div>
        </div>
        {isHost ? <PendingButton busy={busy} type="button" disabled={busy} onClick={() => act(() => initialize(rounds))} className="button-primary mt-5 min-h-12 w-full text-lg">ゲームを始める →</PendingButton> : <p className="mt-5 border-2 border-dashed border-[#b9b5a8] p-4 text-center font-bold text-[var(--muted)]">ホストがラウンド数を決めています…</p>}
        {isHost && <PendingButton busy={busy} type="button" onClick={() => void act(onBackToLobby)} className="mt-4 min-h-11 w-full font-black text-[var(--muted)] underline underline-offset-4">ロビーへ戻る</PendingButton>}
  ...
  if (state.phase === 'finished') return (
    <GameWrapper headerActions={headerActions} players={roomState.players} myUserId={myUserId}>
      <section className="pinch-game mx-auto w-full max-w-2xl border-2 border-[var(--line)] bg-[var(--surface)] p-4 shadow-[5px_5px_0_var(--line)] sm:p-7"><GameHeading subtitle="FINAL SCORE">最終ランキング</GameHeading><Ranking state={state} players={roomState.players} /><ErrorMessage message={error} />{isHost && <PendingButton busy={busy} type="button" onClick={() => void act(onBackToLobby)} className="button-primary mt-6 min-h-12 w-full">ロビーへ戻る</PendingButton>}</section>
    </GameWrapper>
  );
