...
export function ResultPhase({ roomId, myUserId, players, gameState, isHost, onResetGame }: ResultPhaseProps) {
  ...
  const { pending: isResetting, acquire: acquireIsResetting, release: releaseIsResetting } = useActionLock();
  const [resetError, setResetError] = useState<string | null>(null);

  const handleReset = async () => {
    if (isResetting) return;

    if (!acquireIsResetting()) return;
    setResetError(null);
    try {
      await onResetGame();
    } catch (error) {
  ...
  return (
    <div className="text-white mt-3 sm:mt-8">
      ...
      <div className="max-w-2xl mx-auto w-full border-t border-slate-600 pt-8 mt-8">
        {isHost ? (
          <PendingButton busy={isResetting} pendingLabel="リセット中…"
