...
export function GuestNameInput() {
  ...
  const changeName = (value: string) => {
    ...
    try {
      saveGuestDisplayProfile({ name: value.trim(), avatar: profile.avatar });
      setSaveError(false);
    } catch {
      setSaveError(true);
    }
    ...
    <div className="flex w-full min-w-0 items-start gap-3">
      <div className="shrink-0 pt-5">
        {loading ? (
          ...
          <Avatar avatarUrl={profile?.avatar} name={name || 'ゲスト'} size="lg" />
        )}
      </div>
      <div className="min-w-0 flex-1">
        ...
        <p id={`${inputId}-help`} className="mt-2 text-xs leading-5 text-[var(--muted)]" aria-live="polite">
          {error
            ...
            : saveError
              ? '名前を保存できませんでした。もう一度入力してください。'
              : '名前はこの端末に自動保存され、入室時に使われます。'}
        </p>
      </div>
    </div>
  );
